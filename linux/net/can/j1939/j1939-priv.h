...
struct j1939_sk_buff_cb {
	...
	//BAM Delay Mods
	//u8 tpflags; 				//Flags for modifying the trasnport protocol
	//#define BAM_NODELAY 	1	//One such flag
};
#define BAM_NODELAY 		0x80000 //Seems to be unused
...
static inline int j1939cb_use_bamdelay(const struct j1939_sk_buff_cb *skcb)
{
	...
	//printk(KERN_ALERT "DEBUG: skcb->msg_flags & BAM_NODELAY state: %d\n",skcb->msg_flags & BAM_NODELAY);

	if(skcb->msg_flags & BAM_NODELAY)
	{
		return 0;
	}
